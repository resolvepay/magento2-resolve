...
class MassAction extends \Resolve\Resolve\Controller\Adminhtml\Rule
{
    public function execute()
    {
        ...
        if ($ids && in_array($action, ['activate', 'inactivate', 'delete'])) {
            try {
                ...
                switch($action) {
                    case 'delete':
                        $collection = $this->_objectManager->create('Resolve\Resolve\Model\ResourceModel\Rule\Collection');
                ...
                if($status > -1) {
                    $this->_objectManager->create('Resolve\Resolve\Model\Rule')->massChangeStatus($ids, $status);
                }
