...
use Resolve\Resolve\Model\Config as ResolveConfig;

/**
 * Class Checkout for Resolve
 * This class is a wrapper for the Resolve checkout process
 *
...
class Checkout
{
    ...
    public function __construct(
        ...
        if (isset($params['config']) && $params['config'] instanceof ResolveConfig) {
    ...
    public function place($token)
    {
        if (!$this->quote->getGrandTotal()) {
            throw new \Magento\Framework\Exception\LocalizedException(
                __(
                    'Resolve can\'t process orders with a zero balance due. '
        ...
        if (!$token) {
            throw new \Magento\Framework\Exception\LocalizedException(
                __(
                    'Token is absent, some problem with response from Resolve happened.'
                )
            );
        }
