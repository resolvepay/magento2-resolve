<?php
namespace Resolve\Resolve\Model\Rule\Condition;
...
class Combine extends \Magento\Rule\Model\Condition\Combine
{
    ...
    )
    {
        ...
        $this->setType('Resolve\Resolve\Model\Rule\Condition\Combine');
    }

    public function getNewChildSelectOptions()
    {
        $addressCondition = $this->objectManager->create('Resolve\Resolve\Model\Rule\Condition\Address');
        $addressAttributes = $addressCondition->loadAttributeOptions()->getAttributeOption();

        $attributes = array();
        foreach ($addressAttributes as $code=>$label) {
            $attributes[] = array('value'=>'Resolve\Resolve\Model\Rule\Condition\Address|'.$code, 'label'=>$label);
        }

        $conditions = parent::getNewChildSelectOptions();
        $conditions = array_merge_recursive($conditions, array(
            array('value' => 'Magento\SalesRule\Model\Rule\Condition\Product\Found', 'label'=>__('Product attribute combination')),
            array('value' => 'Resolve\Resolve\Model\Rule\Condition\Product\Subselect', 'label'=>__('Products subselection')),
